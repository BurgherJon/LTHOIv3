...
public class League_Season
{
    ...
    public League_Season(String user_id, int league_season_id)
    {
        String strquery;
        ...
        try
        {
            ...
            if (rs.next()) //Anything in the result set?
            {
                ...
                //This method retreives the current players record in this league_season based on an already set league_season_id and the passed user_id
                this.retrieveNum_Players();

                //retrieve a record for the league_season and store it.
                Record tempRecord = new Record(user_id, this.getLeague_Season_ID());
    ...
    public void retrieveNum_Players()
    {
        String strquery;
        final Logger log = Logger.getLogger(League_Season.class.getName());
